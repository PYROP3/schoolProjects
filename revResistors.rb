def simplifyNum num, digits, float
    inp = num.to_s
    out = ""
    counting = 0
    found = false
    ...
    inp.each_char { |c| 
        if (!found && validF.include?(c))
            ...
            elsif (found && c == "." && counting < digits && float)
                  out << c
        end
    }
    return out.to_i if !float
      return out.to_f if float
end
...
while true
    ...
    while !tolerance.has_key?(tol) do
        print "Tolerance (0 => +/-20% by default): "
        tol = gets.chomp.to_f
    ...
    while !mathsIsOk
        ...
        case bNum
            when 3, 4
                val = simplifyNum(res, 2, false)
                res = simplifyNum(res, 2, true)
                res = res*10**(removeDots(oldRes).to_s.length - removeDots(res).to_s.length)
                #puts "Val: " + val.to_s
                #puts "Res: " + res.to_s
                ...
                #puts "Cal: " + simplifyNum(mul*val, 2, true).to_s
                #puts "Res: " + res.to_s
                #puts "Old res: " + oldRes.to_s
                mathsIsOk = simplifyNum(mul*val, 2, true)/res % 10 == 0 || res/simplifyNum(mul*val, 2, true) % 10 == 0 || res == oldRes
            ...
            when 5, 6
                val = simplifyNum(res, 3, false)
                        res = simplifyNum(res, 3, true)
                        res = res*10**(removeDots(oldRes).to_s.length - removeDots(res).to_s.length)
                mul = (res/val)
                if mul != mul.ceil
                    approx = true
                ...
                #mathsIsOk = simplifyNum(mul*val, 3, true) == res
                mathsIsOk = simplifyNum(mul*val, 3, true)/res % 10 == 0 || res/simplifyNum(mul*val, 3, true) % 10 == 0 || res == oldRes
    ...
    puts "Maths failed; RESISTOR does not exist. Approximated resistance value " + oldRes.to_s + " to " + res.to_s unless (oldRes/res)%10 == 0 || (res/oldRes)%10 == 0 || oldRes == res
    puts "\n--\\--\n"
end